...
$(function() {
	$("#dg").datagrid({
		...
		columns:[[  
	        ...
	        {field:'managerId',title:'班主任',width:100,align:'center'},
		...
		toolbar: [{
			...
			iconCls: 'icon-undo',
			handler: function() {
				location.href = "classesListen/downloadAll";
			}
		}, '-', {
			text: "导出查询全部",
			iconCls: 'icon-undo',
			handler: function() {
				location.href = "classesListen/downloadPart";
			}
		}, '-', {
			text: "导入",
			iconCls: 'icon-redo',
			...
			iconCls: 'icon-undo',
			handler: function() {
				location.href = "classesListen/downloadMode?fileName=试听班级模板.xls";
			}
		}]
	});
...
function query(){
	var classesName = $("#search_classesName").val();
	var param = {"classesName":classesName};
