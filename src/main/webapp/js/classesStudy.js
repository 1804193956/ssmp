...
$(function() {
	$("#dg").datagrid({
		...
		toolbar: [{
			...
			iconCls: 'icon-undo',
			handler: function() {
				location.href = "classesStudy/downloadAll";
			}
		}, '-', {
			text: "导出查询全部",
			iconCls: 'icon-undo',
			handler: function() {
				...
				location.href = "classesStudy/downloadPart?ids="+ids;
			}
		}, '-', {
			text: "导入",
			iconCls: 'icon-redo',
			...
			iconCls: 'icon-undo',
			handler: function() {
				location.href = "classesStudy/downloadMode?fileName=正式班级模板.xls";
			}
		}]
	});
